...
export default function SuzukiHonda() {
  ...
  const [hondaData, setHondaData] = useState([
    {
      ...
      date: '2024-10-01',
      vehicleNo: 'DHA-2468',
      dealerName: 'Rahman Motors',
      doSL: 'DO-001',
      coU: 'CO-101',
      destination: 'Chittagong',
      bikeQty: 25,
      unloadCharge: 5000,
      vehicleRentWithVATTax: 22000,
      totalAmount: 27000
    },
    {
      id: 2,
      date: '2024-10-02',
      vehicleNo: 'DHA-1357',
      dealerName: 'Rahim Trading',
      doSL: 'DO-002',
      coU: 'CO-102',
      destination: 'Sylhet',
      bikeQty: 30,
      unloadCharge: 6000,
      vehicleRentWithVATTax: 25000,
      totalAmount: 31000
    },
    {
      id: 3,
      date: '2024-10-03',
      vehicleNo: 'DHA-9753',
      dealerName: 'Ali Brothers',
      doSL: 'DO-003',
      coU: 'CO-103',
      destination: 'Khulna',
      bikeQty: 20,
      unloadCharge: 4500,
      vehicleRentWithVATTax: 20000,
      totalAmount: 24500
    }
  ]);

  // Calculate totals for stats
  const totalAmount = hondaData.reduce((sum, item) => sum + item.totalAmount, 0);
  const totalTrips = hondaData.length;
  ...
  const handleUpdate = () => {
    setHondaData(hondaData.map(item => 
  ...
  const handleDelete = (id) => {
    if (window.confirm('Are you sure you want to delete this record?')) {
      setHondaData(hondaData.filter(item => item.id !== id));
    }
  };
  ...
  const filteredData = hondaData.filter(item => {
    const searchStr = searchTerm.toLowerCase();
    return item.vehicleNo.toLowerCase().includes(searchStr) ||
           item.dealerName.toLowerCase().includes(searchStr) ||
      ...
      <div className="bg-gradient-to-r from-blue-900 to-blue-800 px-8 py-4 shadow-lg">
        <div className="flex items-center justify-between">
          ...
          <div className="flex items-center gap-2">
            <h1 className="text-white text-xl font-semibold">Suzuki Honda</h1>
          </div>
        </div>
      </div>
        ...
        <div className="bg-white rounded-lg shadow-sm border border-gray-100">
          ...
          <div className="p-6 border-b border-gray-200">
            <div className="flex items-center justify-between mb-4">
              <h2 className="text-xl font-bold text-gray-800">Suzuki Honda Records</h2>
              <Link to="/hondabillrecord" className="flex items-center gap-2 px-4 py-2 bg-blue-600 text-white rounded-lg hover:bg-blue-700 transition-colors">
                <Plus size={18} />
            ...
            <table className="w-full">
              <thead className="bg-blue-900 text-white">
                <tr>
                  ...
                  <th className="px-4 py-3 text-left text-sm font-semibold">DO (SL)</th>
                  ...
                  <th className="px-4 py-3 text-left text-sm font-semibold">Bike Qty</th>
                  <th className="px-4 py-3 text-left text-sm font-semibold">Unload Charge</th>
                  <th className="px-4 py-3 text-left text-sm font-semibold">Vehicle Rent (with VAT+Tax)</th>
                ...
                {currentPageItems.length > 0 ? (
                  currentPageItems.map((item, idx) => (
                    <tr key={item.id} className="hover:bg-gray-50">
                      ...
                      <td className="px-4 py-3 text-sm">{item.dealerName}</td>
                      <td className="px-4 py-3 text-sm">{item.doSL}</td>
                      <td className="px-4 py-3 text-sm">{item.coU}</td>
                      <td className="px-4 py-3 text-sm">{item.destination}</td>
                      <td className="px-4 py-3 text-sm">{item.bikeQty}</td>
                      <td className="px-4 py-3 text-sm">৳{item.unloadCharge.toLocaleString()}</td>
                      <td className="px-4 py-3 text-sm">৳{item.vehicleRentWithVATTax.toLocaleString()}</td>
            ...
            <div className="p-6">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
                <div>
                  ...
                  <input
                    type="text"
                    value={editingItem.dealerName}
                    onChange={(e) => setEditingItem({...editingItem, dealerName: e.target.value})}
                    className="w-full px-4 py-2.5 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:outline-none"
                  />
                </div>
                <div>
                  <label className="block text-sm font-medium text-gray-700 mb-2">DO (SL)</label>
                  <input
                    type="text"
                    value={editingItem.doSL}
                    onChange={(e) => setEditingItem({...editingItem, doSL: e.target.value})}
                    className="w-full px-4 py-2.5 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:outline-none"
                  />
                </div>
              </div>
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
                <div>
                  ...
                  <input
                    type="text"
                    value={editingItem.coU}
                    onChange={(e) => setEditingItem({...editingItem, coU: e.target.value})}
                    className="w-full px-4 py-2.5 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:outline-none"
                  />
                </div>
                <div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
                <div>
                  ...
                  <input
                    type="number"
                    value={editingItem.bikeQty}
                    onChange={(e) => setEditingItem({...editingItem, bikeQty: Number(e.target.value)})}
                    className="w-full px-4 py-2.5 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:outline-none"
                  />
                </div>
                <div>
                  ...
                  <input
                    type="number"
                    value={editingItem.unloadCharge}
                    onChange={(e) => setEditingItem({...editingItem, unloadCharge: Number(e.target.value)})}
                    className="w-full px-4 py-2.5 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:outline-none"
                  />
                </div>
              </div>
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
                <div>
                  ...
                  <input
                    type="number"
                    value={editingItem.vehicleRentWithVATTax}
                    onChange={(e) => setEditingItem({...editingItem, vehicleRentWithVATTax: Number(e.target.value)})}
                    className="w-full px-4 py-2.5 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:outline-none"
                  />
                </div>
                <div>
