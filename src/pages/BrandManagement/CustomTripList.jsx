  ...
  useEffect(() => {
    const fetchInitialData = async () => {
      try {
        ...
        const customerId = productData.note.company_id;
        console.log(productData)
        
        // 2. Fetch customer details
      
        const customerRes = await fetch(`${API_BASE_URL}/customer/${customerId}`);
        if (!customerRes.ok) throw new Error('Failed to fetch customer');
        const customerData = await customerRes.json();
        
        setCustomer({
          id: customerData.id,
          name: customerData.note?.customerName || customerData.customerName || 'Unknown',
        ...
        // 5. Fetch trips for customer
        const tripsRes = await fetch(`${API_BASE_URL}/trip-fields?customerId=${customerId}`);
        if (tripsRes.ok) {
...
console.log(id)
  ...
  const fetchSelectedFields = async (productId) => {
    try {
      ...
      if (response.ok) {
        ...
        console.log('Fetched trip fields:', data);
        
        // Handle different response formats
        if (Array.isArray(data)) {
          setSelectedFields(data);
        } else if (data.trip_fields && Array.isArray(data.trip_fields)) {
          setSelectedFields(data.trip_fields);
        } else {
          setSelectedFields([]);
        }
      } else {
        console.warn('No trip fields found for product:', productId);
        setSelectedFields([]);
      }
    } catch (err) {
  ...
  const filteredTrips = trips
    .filter(trip => trip.category === activeProductId || trip.productId === activeProductId)
    .filter(trip => {
      const search = searchTerm.toLowerCase();
      return tableColumns.some(col =>
        trip[col.key]?.toString().toLowerCase().includes(search)
      );
    })
    .filter(trip => {
  ...
  const totalAmount = filteredTrips.reduce((sum, item) => 
    sum + (Number(item.totalAmount) || Number(item.total) || 0), 0
  );
