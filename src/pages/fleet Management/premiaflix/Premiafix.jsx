...
import {
  ...
  Edit2, Trash2, AlertCircle, Eye,
  ChevronLeft, ChevronRight, X, TrendingUp, DollarSign
} from 'lucide-react';
import { Link } from 'react-router-dom';

export default function Premiafix() {
  ...
  // Premiafix Data
  const [premiafixData, setPremiafixData] = useState([
    {
      id: 1,
      date: '2024-10-01',
      vehicleNo: 'DHA-1111',
  ...
  const totalAmount = premiafixData.reduce((sum, item) => sum + item.totalAmount, 0);
  const totalTrips = premiafixData.length;
  ...
  const handleUpdate = () => {
    setPremiafixData(premiafixData.map(item =>
  ...
  const handleDelete = (id) => {
    if (window.confirm('Are you sure you want to delete this record?')) {
      setPremiafixData(premiafixData.filter(item => item.id !== id));
    }
  };
  ...
  const filteredData = premiafixData.filter(item => {
      ...
      <div className="bg-gradient-to-r from-blue-900 to-blue-800 px-8 py-4 shadow-lg">
        <div className="flex items-center justify-between">
          ...
          <div className="flex items-center gap-2">
            <h1 className="text-white text-xl font-semibold">Premiafix</h1>
          </div>
        </div>
      </div>
        ...
        <div className="bg-white rounded-lg shadow-sm border border-gray-100">
          ...
          <div className="p-6 border-b border-gray-200">
            <div className="flex items-center justify-between mb-4">
              <h2 className="text-xl font-bold text-gray-800">Premiafix Records</h2>
                ...
                {currentPageItems.length > 0 ? (
                  currentPageItems.map((item, idx) => (
                    <tr key={item.id} className="hover:bg-gray-50">
                      ...
                      <td className="px-4 py-3 text-sm">{item.quantity} KG</td>
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-lg shadow-xl max-w-4xl w-full max-h-[90vh] overflow-y-auto">
            ...
            <div className="p-6 grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-2">Quantity (KG)</label>
                <p className="mt-1 text-sm text-gray-900">{viewingItem.quantity} KG</p>
              </div>
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-2">Vehicle Rent</label>
                <p className="mt-1 text-sm text-gray-900">৳{viewingItem.vehicleRent.toLocaleString()}</p>
              </div>
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-2">Dropping</label>
                <p className="mt-1 text-sm text-gray-900">৳{viewingItem.dropping.toLocaleString()}</p>
              </div>
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-2">5% ALT</label>
                <p className="mt-1 text-sm text-gray-900">৳{viewingItem.alt5.toLocaleString()}</p>
              </div>
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-2">10% VAT</label>
                <p className="mt-1 text-sm text-gray-900">৳{viewingItem.vat10.toLocaleString()}</p>
              </div>
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-2">Total Amount</label>
