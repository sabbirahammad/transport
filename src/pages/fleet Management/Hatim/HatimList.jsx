...
export default function HatimList() {
  ...
  const totalAmount = hatimData.reduce((sum, item) => sum + item.totalAmount, 0);
  const totalTrips = hatimData.length;
  ...
  const handleUpdate = () => {
    setHatimData(hatimData.map(item =>
  ...
  const handleDelete = (id) => {
    if (window.confirm('Are you sure you want to delete this record?')) {
      setHatimData(hatimData.filter(item => item.id !== id));
    }
  };
  ...
  const filteredData = hatimData.filter(item => {
    const searchStr = searchTerm.toLowerCase();
    return item.vehicleNo.toLowerCase().includes(searchStr) ||
            item.loadPoint.toLowerCase().includes(searchStr) ||
            item.unloadPoint.toLowerCase().includes(searchStr);
  });
  ...
  const exportToExcel = () => {
    const data = selectedItems.length > 0
      ? hatimData.filter(item => selectedItems.includes(item.id))
      : hatimData;
    ...
    const worksheet = XLSX.utils.json_to_sheet(data.map(item => ({
      ...
      'Load Point': item.loadPoint,
      'Unload Point': item.unloadPoint,
    ...
    XLSX.utils.book_append_sheet(workbook, worksheet, 'Hatim Records');
    XLSX.writeFile(workbook, 'hatim_records.xlsx');
  };
  ...
  const exportToPDF = () => {
    ...
    const head = [['ID', 'Date', 'Vehicle No', 'Load Point', 'Unload Point', 'Vehicle Size', 'Advance', 'Due', 'Total', 'Total Amount', 'Profit']];
      ...
      <div className="bg-gradient-to-r from-blue-900 to-blue-800 px-8 py-4 shadow-lg">
        <div className="flex items-center justify-between">
          ...
          <div className="flex items-center gap-2">
            <h1 className="text-white text-xl font-semibold">Hatim</h1>
          </div>
        </div>
      </div>
        ...
        <div className="bg-white rounded-lg shadow-sm border border-gray-100">
          ...
          <div className="p-6 border-b border-gray-200">
            <div className="flex items-center justify-between mb-4">
              <h2 className="text-xl font-bold text-gray-800">Hatim Records</h2>
                ...
                {currentPageItems.length > 0 ? (
                  currentPageItems.map((item, idx) => (
                    <tr key={item.id} className="hover:bg-gray-50">
                      ...
                      <td className="px-4 py-3 text-sm">{item.loadPoint}</td>
                      <td className="px-4 py-3 text-sm">{item.unloadPoint}</td>
                      ...
                      <td className="px-4 py-3 text-sm">৳{item.total.toLocaleString()}</td>
                      <td className="px-4 py-3 text-sm font-semibold text-green-600">৳{item.totalAmount.toLocaleString()}</td>
          ...
          <div className="bg-white rounded-lg shadow-xl max-w-4xl w-full max-h-[90vh] overflow-y-auto">
            ...
            <div className="p-6 grid grid-cols-1 md:grid-cols-2 gap-4">
              <div>
                ...
                <p className="mt-1 text-sm text-gray-900">{viewingItem.date}</p>
              </div>
              <div>
              ...
              <div>
                ...
                <p className="mt-1 text-sm text-gray-900">{viewingItem.loadPoint}</p>
              </div>
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-2">Unload Point</label>
                <p className="mt-1 text-sm text-gray-900">{viewingItem.unloadPoint}</p>
              </div>
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-2">Vehicle Size</label>
                <p className="mt-1 text-sm text-gray-900">{viewingItem.vehicleSize}</p>
              </div>
              <div>
              ...
              <div>
                ...
                <p className="mt-1 text-sm text-gray-900">৳{viewingItem.totalAmount.toLocaleString()}</p>
              </div>
              <div>
            ...
            <div className="p-6">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
                <div>
                  ...
                  <input
                    type="text"
                    value={editingItem.loadPoint}
                    onChange={(e) => setEditingItem({...editingItem, loadPoint: e.target.value})}
                    className="w-full px-4 py-2.5 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:outline-none"
                  />
                </div>
                <div>
                  ...
                  <input
                    type="text"
                    value={editingItem.unloadPoint}
                    onChange={(e) => setEditingItem({...editingItem, unloadPoint: e.target.value})}
                    className="w-full px-4 py-2.5 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:outline-none"
                  />
                </div>
              </div>
