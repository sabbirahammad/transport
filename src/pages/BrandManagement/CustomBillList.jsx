...
//       if (stateCustomerName) setcustomerName(stateCustomerName);
...
//   // All possible fields definition (matching the actual JSON model from API)
//   const allFields = [
//     { key: 'id', label: 'ID' },
...
//     { key: 'vehicle_size', label: 'Vehicle Size' },
//     { key: 'from', label: 'From' },
...
//     { key: 'bike_qty', label: 'Bike Qty' },
//     { key: 'unload_charge', label: 'Unload Charge' },
//     { key: 'vehicle_rent_with_vat_tax', label: 'Vehicle Rent with VAT Tax' },
...
//       const label = cleanFieldKey
//         .split('_')
//         .map(part => part.charAt(0).toUpperCase() + part.slice(1))
//         .join(' ');

//       return { key: cleanFieldKey, label };
//     });
...
//                 ) : (
//                   <tr>
//                     <td colSpan={validSelectedFields.length + 3} className="px-4 py-12 text-center text-gray-500">
...
console.log(productName)
  ...
  // All possible fields definition (matching both selectedFields format and JSON model)
  const allFields = [
    { key: 'id', label: 'ID', dataKey: 'id' },
    { key: 'category', label: 'Category', dataKey: 'category' },
    { key: 'date', label: 'Date', dataKey: 'date' },
    { key: 'challanNo', label: 'Challan No', dataKey: 'challan_no' },
    { key: 'status', label: 'Status', dataKey: 'status' },
    { key: 'customerName', label: 'Customer Name', dataKey: 'customer_name' },
    { key: 'distributorName', label: 'Distributor Name', dataKey: 'distributor_name' },
    { key: 'dealerName', label: 'Dealer Name', dataKey: 'dealer_name' },
    { key: 'vehicleNo', label: 'Vehicle No', dataKey: 'vehicle_no' },
    { key: 'driverName', label: 'Driver Name', dataKey: 'driver_name' },
    { key: 'vehicleSize', label: 'Vehicle Size', dataKey: 'vehicle_size' },
    { key: 'from', label: 'From', dataKey: 'from' },
    { key: 'destination', label: 'Destination', dataKey: 'destination' },
    { key: 'portfolio', label: 'Portfolio', dataKey: 'portfolio' },
    { key: 'product', label: 'Product', dataKey: 'product' },
    { key: 'goods', label: 'Goods', dataKey: 'goods' },
    { key: 'quantity', label: 'Quantity', dataKey: 'quantity' },
    { key: 'bikeQty', label: 'Bike Qty', dataKey: 'bike_qty' },
    { key: 'financial.unloadCharge', label: 'Unload Charge', dataKey: 'unload_charge' },
    { key: 'financial.vehicleRentWithVATTax', label: 'Vehicle Rent with VAT Tax', dataKey: 'vehicle_rent_with_vat_tax' },
    { key: 'financial.vehicleRent', label: 'Vehicle Rent', dataKey: 'vehicle_rent' },
    { key: 'financial.dropping', label: 'Dropping', dataKey: 'dropping' },
    { key: 'financial.alt5', label: 'Alt5', dataKey: 'alt5' },
    { key: 'financial.vat10', label: 'VAT 10%', dataKey: 'vat10' },
    { key: 'financial.totalRate', label: 'Total Rate', dataKey: 'total_rate' },
    { key: 'financial.advance', label: 'Advance', dataKey: 'advance' },
    { key: 'financial.due', label: 'Due', dataKey: 'due' },
    { key: 'financial.total', label: 'Total', dataKey: 'total' },
    { key: 'financial.profit', label: 'Profit', dataKey: 'profit' },
    { key: 'financial.bodyFare', label: 'Body Fare', dataKey: 'body_fare' },
    { key: 'totalAmount', label: 'Total Amount', dataKey: 'total_amount' },
    { key: 'financial.amount', label: 'Amount', dataKey: 'amount' },
    { key: 'financial.fuelCost', label: 'Fuel Cost', dataKey: 'fuel_cost' },
    { key: 'co_number', label: 'CO Number', dataKey: 'co_number' },
    { key: 'do_number', label: 'DO Number', dataKey: 'do_number' },
    { key: 'created_at', label: 'Created At', dataKey: 'created_at' },
    { key: 'product_id', label: 'Product ID', dataKey: 'product_id' }
  ];

  // Get valid selected fields with labels
  const validSelectedFields = selectedFields
    .map(fieldKey => {
      // Try to find exact match first (for selectedFields format)
      const field = allFields.find(f => f.key === fieldKey);
      if (field) return field;

      // If not found, try to find by dataKey
      const dataField = allFields.find(f => f.dataKey === fieldKey);
      if (dataField) return dataField;

      // If still not found, create a field object with formatted label
      ...
      return { key: fieldKey, label, dataKey: fieldKey };
    });
  ...
  const filteredBills = bills
    .filter(bill => {
      if (!searchTerm) return true;
      return validSelectedFields.some(col => {
        const value = getNestedValue(bill, col.dataKey || col.key);
        return String(value || '').toLowerCase().includes(searchTerm.toLowerCase());
      });
    })
    .filter(bill => {
  ...
  const exportToExcel = () => {
    ...
    const worksheet = XLSX.utils.json_to_sheet(data.map(b => {
      const row = { 'Product': productName || '' };
      tableColumns.forEach(col => {
        const value = getNestedValue(b, col.dataKey || col.key);
        row[col.label] = value || '';
      });
      return row;
    }));
      ...
      {showViewModal && viewingBill && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-lg shadow-xl max-w-4xl w-full max-h-[90vh] overflow-y-auto">
            ...
            <div className="p-6 grid grid-cols-1 md:grid-cols-2 gap-4">
              {validSelectedFields.map(field => {
                const value = getNestedValue(viewingBill, field.dataKey || field.key);
                return (
                  <div key={field.key} className="mb-2">
                    <label className="block text-sm font-medium text-gray-700">
                      {field.label}
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-lg shadow-xl max-w-4xl w-full max-h-[90vh] overflow-y-auto">
            ...
            <div className="p-6">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                {validSelectedFields.map(field => {
                  const value = getNestedValue(editingBill, field.dataKey || field.key);
                  return (
                    <div key={field.key}>
                      <label className="block text-sm font-medium text-gray-700 mb-2">
                        {field.label}
                      </label>
                      {field.dataKey === 'date' || field.dataKey === 'created_at' ? (
                        <input
                          type="date"
                          value={value || ''}
                          onChange={(e) => {
                            const updated = { ...editingBill };
                            setNestedValue(updated, field.dataKey || field.key, e.target.value);
                            setEditingBill(updated);
                          }}
                          className="w-full px-4 py-2.5 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500"
                        />
                        ...
                        <input
                          ...
                          onChange={(e) => {
                            ...
                            const val = typeof value === 'number'
                              ? Number(e.target.value)
                              : e.target.value;
                            setNestedValue(updated, field.dataKey || field.key, val);
                            setEditingBill(updated);
                          }}
                          className="w-full px-4 py-2.5 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500"
                        />
