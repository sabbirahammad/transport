...
export default function OunVehicle() {
  ...
  const handleSubmitDateRange = async () => {
    ...
    try {
      setLoadingApi(true);
      const url = `${API_BASE_URL}/own-vehicles/by-driver?driver_name=${encodeURIComponent(selectedVehicle.driverName)}&start_date=${startDate}&end_date=${endDate}`;
      const response = await fetch(url);
      if (!response.ok) {
        throw new Error(`HTTP error! status: ${response.status}`);
      }
      ...
      console.error('Error fetching own vehicles data:', err);
      setApiResponseData({ error: err.message });
    } finally {
      setLoadingApi(false);
    }
  };
  ...
  const exportToExcel = () => {
    ...
    XLSX.utils.book_append_sheet(workbook, worksheet, 'OwnVehicles');
    XLSX.writeFile(workbook, 'own-vehicles.xlsx');
  };
  ...
  const exportToPDF = () => {
    ...
    // Add title
    doc.setFontSize(18);
    doc.text('Own Vehicle List', 14, 22);
    doc.setFontSize(11);
    ...
    // Create table
    autoTable(doc, {
      head: [['SL No', 'Driver Name', 'Vehicle Name', 'Vehicle No']],
      body: tableData,
      startY: 40,
      theme: 'grid',
      ...
      <div className="bg-gradient-to-r from-blue-900 via-blue-800 to-indigo-800 px-4 sm:px-8 py-4 sm:py-6 shadow-2xl">
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-4">
            ...
            <div>
              <h1 className="text-white text-3xl font-bold">Own Vehicle Management</h1>
              <p className="text-blue-100 text-sm">Manage and monitor your own vehicles</p>
            </div>
          </div>
        </div>
      </div>
          ...
          <div className="bg-gradient-to-r from-gray-50 to-blue-50 p-4 sm:p-8 border-b border-gray-200">
            <div className="flex items-center justify-between mb-6">
              <div className="flex items-center gap-4">
                ...
                <div>
                  <h2 className="text-2xl font-bold text-gray-800">Own Vehicle Fleet</h2>
                  <p className="text-gray-600">Manage all your own vehicles in one place</p>
                </div>
              </div>
            ...
            <div className="flex flex-wrap items-center gap-4">
              <div className="flex-1 min-w-[200px] sm:min-w-[300px] relative">
                ...
                <input
                  type="text"
                  placeholder="Search by driver name, vehicle name, or vehicle number..."
                  value={searchTerm}
                  ...
                  <tbody className="divide-y divide-gray-200">
                    {currentPageVehicles.length > 0 ? (
                      currentPageVehicles.map((vehicle, idx) => (
                        <tr key={vehicle.id} className="hover:bg-gradient-to-r hover:from-blue-50 hover:to-indigo-50 transition-all cursor-pointer" onClick={() => handleViewVehicle(vehicle)}>
                          ...
                          <td className="px-6 py-4">
                            <div className="flex items-center justify-center gap-2">
                              <button
                                onClick={(e) => { e.stopPropagation(); handleViewVehicle(vehicle); }}
          ...
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-full sm:max-w-7xl max-h-[90vh] overflow-auto">
            <div className="bg-gradient-to-r from-blue-900 via-blue-800 to-indigo-800 text-white px-4 sm:px-8 py-4 sm:py-6 flex items-center justify-between sticky top-0 rounded-t-2xl">
              <div className="flex items-center gap-4">
                ...
                <div>
                  <h2 className="text-2xl font-bold">Vehicle Details & Trip Data</h2>
                  <p className="text-blue-100 text-sm">View vehicle information and fetch trip data by date range</p>
                </div>
              </div>
              <button
            ...
            <div className="p-4 sm:p-6">
              ...
              <div className="mb-8">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  <div>
                    ...
                    <p className="text-gray-800 font-medium">{selectedVehicle.driverName}</p>
                  </div>
                  <div>
                    <label className="block text-sm font-medium text-gray-700 mb-2">Vehicle Name</label>
                    <p className="text-gray-800 font-medium">{selectedVehicle.vehicleName}</p>
                  </div>
                  <div>
                    <label className="block text-sm font-medium text-gray-700 mb-2">Vehicle No</label>
                    <p className="text-gray-800 font-medium">{selectedVehicle.vehicleNo}</p>
                  </div>
                  <div>
                    <label className="block text-sm font-medium text-gray-700 mb-2">Category</label>
                    <p className="text-gray-800 font-medium">{selectedVehicle.category}</p>
                  </div>
                  <div>
                    <label className="block text-sm font-medium text-gray-700 mb-2">Size</label>
                    <p className="text-gray-800 font-medium">{selectedVehicle.size}</p>
                  </div>
                  <div>
                    <label className="block text-sm font-medium text-gray-700 mb-2">Status</label>
                    <p className="text-gray-800 font-medium">{selectedVehicle.status}</p>
                  </div>
                </div>
              </div>
              ...
              {/* Date Range Section */}
              <div className="mb-8">
                <h3 className="text-lg font-semibold text-gray-800 mb-4">Fetch Trip Data</h3>
                <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                  ...
                  <div className="flex items-end">
                    <button
                      ...
                      disabled={loadingApi || !startDate || !endDate}
                      className="px-6 py-2.5 bg-gradient-to-r from-blue-900 to-blue-800 text-white rounded-lg hover:from-blue-800 hover:to-blue-700 disabled:opacity-50 disabled:cursor-not-allowed"
                    >
                      {loadingApi ? 'Loading...' : 'Submit'}
                    </button>
                  </div>
                </div>
              </div>
              ...
              {apiResponseData && (
                <div className="mb-8">
                  <h3 className="text-lg font-semibold text-gray-800 mb-4">Trip Data Results</h3>
                  <div className="bg-gray-50 p-4 rounded-lg">
                    {apiResponseData.error ? (
                      <p className="text-red-600">Error: {apiResponseData.error}</p>
                    ) : (
