...
export default function MahindraList() {
  ...
  // Mahindra Data
  const [mahindraData, setMahindraData] = useState([
    {
      id: 1,
      date: '2024-10-01',
      cash: 5000,
  ...
  const totalAmount = mahindraData.reduce((sum, item) => sum + item.total, 0);
  const totalTrips = mahindraData.length;
  ...
  const handleUpdate = () => {
    setMahindraData(mahindraData.map(item =>
  ...
  const handleDelete = (id) => {
    if (window.confirm('Are you sure you want to delete this record?')) {
      setMahindraData(mahindraData.filter(item => item.id !== id));
    }
  };
  ...
  const filteredData = mahindraData.filter(item => {
      ...
      <div className="bg-gradient-to-r from-blue-900 to-blue-800 px-8 py-4 shadow-lg">
        <div className="flex items-center justify-between">
          ...
          <div className="flex items-center gap-2">
            <h1 className="text-white text-xl font-semibold">Mahindra</h1>
          </div>
        </div>
      </div>
        ...
        <div className="bg-white rounded-lg shadow-sm border border-gray-100">
          ...
          <div className="p-6 border-b border-gray-200">
            <div className="flex items-center justify-between mb-4">
              <h2 className="text-xl font-bold text-gray-800">Mahindra Records</h2>
            ...
            <div className="flex items-center gap-3">
              <div className="flex-1 relative">
                ...
                <input
                  type="text"
                  placeholder="Search by vehicle no, destination, driver..."
                  value={searchTerm}
                ...
                {currentPageItems.length > 0 ? (
                  currentPageItems.map((item, idx) => (
                    <tr key={item.id} className="hover:bg-gray-50">
                      ...
                      <td className="px-4 py-3 text-sm">৳{item.cash.toLocaleString()}</td>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
                ...
                <div>
                  ...
                  <input
                    ...
                    onChange={(e) => setEditingItem({...editingItem, cash: Number(e.target.value)})}
                    className="w-full px-4 py-2.5 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:outline-none"
                  />
                </div>
              </div>
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-lg shadow-xl max-w-4xl w-full max-h-[90vh] overflow-y-auto">
            ...
            <div className="p-6 grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-2">Cash</label>
                <p className="mt-1 text-sm text-gray-900">৳{viewingItem.cash.toLocaleString()}</p>
              </div>
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-2">Destination</label>
